...
import { getDiscountedPrice, getDiscountRate, usePromoStatus } from '../promo';
...
function ServiceCard({ service, t, promo, compact = false }) {
  const promoLive = promo?.isLive;
  return (
      ...
      <div className="package-card-body">
        <div className="package-topline">
          ...
          <div className="topline-badges">
            ...
            {promoLive && <b className="promo-pill">-{Math.round(getDiscountRate(service) * 100)}%</b>}
          </div>
        </div>
        {promoLive
          ? <div className="package-price promo"><s className="price-was">{service.price}</s><span className="price-now">{getDiscountedPrice(service)}</span></div>
...
function RequestForm({ selected, promo }) {
  const { t } = useI18n();
  ...
  async function submit(event) {
    ...
    try {
      const response = await fetch('/api/project-request', {
        ...
        body: JSON.stringify({ name: data.get('name'), email: data.get('email'), phone: data.get('phone'), business: t(data.get('business')), company: data.get('company'), website: data.get('website'), packageName: t(selected.name), packagePrice: promo?.isLive ? `${getDiscountedPrice(selected)} (launch offer, was ${selected.price})` : selected.price, packageSlug: selected.slug }),
      });
      ...
      <section className="package-detail-hero section-pad">
        <div className="package-detail-copy">
          ...
          <div className="detail-price">
            <span>{t('Starting from')}</span>
            {promo.isLive
              ? <div className="detail-price-row"><s className="price-was">{service.price}</s><strong className="price-now">{getDiscountedPrice(service)}</strong><b className="promo-pill">-{Math.round(getDiscountRate(service) * 100)}%</b></div>
  ...
  return (
    <main>
      ...
      <section className="packages section-pad" id="packages">
        ...
        <div className="package-grid" role="tabpanel">{services[category].map((service) => <ServiceCard key={service.slug} service={service} t={t} promo={promo} />)}</div>
